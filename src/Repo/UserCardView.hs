...
returnFields :: Query
returnFields = " num_correct_trials, next_request, user_id, user_deck_id, id, moves, title, color, fen, likelihood "
...
insertOrUpdate time unmarked = one =<< runQuery query (card.ucvId , card.userDeckId, card.numCorrectTrials, card.nextRequest, card.moves, card.title, card.color, card.fen, card.likelihood, card.userId, time, time, card.likelihood)
  where
    query :: Query
    query = "INSERT INTO user_card_views \
    \(id, user_deck_id, \
    \num_correct_trials, next_request, \
    \ moves, title, color, fen, likelihood, user_id, \
    ...
    \, fen = EXCLUDED.fen \
    \, likelihood = COALESCE(?, user_card_views.likelihood) \
    \, next_request = EXCLUDED.next_request \
    \, last_modified = EXCLUDED.last_modified \
    \RETURNING" <> returnFields
    card = mark unmarked

isFeasible :: Integer -> Integer -> Integer -> Integer -> Integer -> Bool
isFeasible now _ _ t _ | t <= 0 = True
isFeasible now p n _ _ | n < p  = False
isFeasible now prevRequest nextRequest trialsGain numCorrectTrials = feasibleNrTrials && feasibleLastTry
  where
    lowerBound 0                    = 0
    ...
    maxLastInterval  num | num <= 1 = 2 * 1000 * 60 * 60 * 24 
    maxLastInterval  num            = (num-1) * 2 * 1000 * 60 * 60 * 24 
    feasibleNrTrials                = (nextRequest - prevRequest) >= lowerBound trialsGain
    feasibleLastTry                 = (nextRequest - maxLastInterval numCorrectTrials) <= now
...
infeasibleUpdated now unmarked = do
  ...
  return $ not $ isFeasible now card.nextRequest proposal.nextRequest trials proposal.numCorrectTrials
  where
