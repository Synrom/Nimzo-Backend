  ...
  describe "claimAnonymousForUser edge cases" $ do
    ...
    it "is idempotent when claimed repeatedly by the same user" $ do
      withCleanDb $ \conn -> do
        ...
        let payload = AnonymousOnboardingProgressPayload "idempotent-session" "elo" True (Just "beginner") (Just "0-800") Nothing Nothing Nothing Nothing Nothing
    ...
    it "fails when a different user tries to claim an already claimed session" $ do
      withCleanDb $ \conn -> do
        ...
        let payload = AnonymousOnboardingProgressPayload "conflict-session" "study_goal" True (Just "advanced") (Just "2000+") Nothing Nothing (Just "20+ mins") Nothing Nothing
        _ <- runTestApp conn $ OnboardingRoutes.saveAnonymousOnboardingProgress payload
